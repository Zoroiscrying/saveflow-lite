...
static func save_template(template_id :int, template :String) -> void:
	if template_id != TEMPLATE_ID_GD and template_id != TEMPLATE_ID_CS:
		...
		return
	if template_id == TEMPLATE_ID_GD:
		GdUnitSettings.saveflow_property(GdUnitSettings.TEMPLATE_TS_GD, template.dedent().trim_prefix("\n"))
	elif template_id == TEMPLATE_ID_CS:
		GdUnitSettings.saveflow_property(GdUnitSettings.TEMPLATE_TS_CS, template.dedent().trim_prefix("\n"))
...
static func reset_to_default(template_id :int) -> void:
	if template_id != TEMPLATE_ID_GD and template_id != TEMPLATE_ID_CS:
		...
		return
	if template_id == TEMPLATE_ID_GD:
		GdUnitSettings.saveflow_property(GdUnitSettings.TEMPLATE_TS_GD, default_GD_template())
	else:
		GdUnitSettings.saveflow_property(GdUnitSettings.TEMPLATE_TS_CS, default_CS_template())
