...
Open `res://addons/saveflow_lite/demo/plugin_sandbox/plugin_sandbox.tscn` to try the current SaveFlow Lite sandbox.
Open `res://addons/saveflow_lite/demo/complex_sandbox/complex_sandbox.tscn` to pressure SaveFlow with a more realistic mid-size save environment.
Open `res://addons/saveflow_lite/demo/zelda_like/scenes/zelda_like_sandbox.tscn` to run a room-switching Zelda-like sandbox with room physics, animation state, room tables, and runtime entity restore.
Open `res://addons/saveflow_lite/demo/recommended_template/scenes/recommended_template_sandbox.tscn` to see the smallest recommended integration template that combines node, system, and entity collection saves in one scene.
...
See [zelda_like/README.md](addons/saveflow_lite/demo/zelda_like/README.md) for the intent of each folder.
